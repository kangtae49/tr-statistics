...
async readFile(path: string) : Promise<Result<string, ApiError>> {
    try {
...
async readFileInfo(path: string) : Promise<Result<FileInfo, ApiError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("read_file_info", { path }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
